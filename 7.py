...
for program in inputs:
	if "->" in program:
		children = program.split(">")[1].strip().split(",")
		children = [child.strip() for child in children]
		all_children += children
		all_nodes.append(program.split(" ")[0])

for node in all_nodes:
	if not node in all_children:
		print "step1: "+node
		bottom_program = node

all_programs = {}
for program in inputs:
	current_program = {"name":"", "children": [], "weight": 0, "actual_weight":0, "child_weight":0, "parent":"", "depth": 0}
	if "->" in program:
		children = program.split(">")[1].strip().split(",")
		children = [child.strip() for child in children]
		current_program["children"] = children
	current_program["weight"] = int(program.split("(")[1].split(")")[0])
	current_program["name"] = program.split(" ")[0]
	all_programs[current_program["name"]] = current_program

def add_parents(program, depth=0, parent=""):
	all_programs[program["name"]]["parent"] = parent
	all_programs[program["name"]]["depth"] = depth
	if len(program["children"]) > 0:
		for child in program["children"]:
			add_parents(all_programs[child], depth+1, program["name"])

def get_weight(program, top, weight=0):
	all_programs[top["name"]]["child_weight"] += weight
	if len(program["children"]) > 0:
		for child in program["children"]:
			get_weight(all_programs[child], top, all_programs[child]["weight"])

add_parents(all_programs[bottom_program])

def printtower(program, depth=0):
	out = ""
	if depth > 0:
		out = " "*2*depth+"|----> "
	out += program["name"] + "("+str(program["child_weight"]+program["weight"])+" ("+str(program["weight"])+"))"
	#print out
	goal_weight = 0
	if len(program["children"]) > 0:
		for child in program["children"]:
			weight = all_programs[child]["child_weight"]+all_programs[child]["weight"]
			if goal_weight == 0:
				goal_weight = weight
			else:
				if not weight == goal_weight:
					# Lowest print will be solution, it's the furthest "into" the tree
					print "step2: "+str(all_programs[child]["weight"]-(weight-goal_weight))
			printtower(all_programs[child], depth+1)

for program in all_programs:
	current_program = all_programs[program]
	get_weight(current_program, current_program)

print "The lowest of the following will be the answer #lazy"
printtower(all_programs[bottom_program])
