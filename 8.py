...
for input in inputs:
	...
	reg_from = parts[0]
	instruction = parts[1]
	instruction_check_num = int(parts[2])
	reg_to = parts[4]
	modifier_check = parts[5]
	modifier_check_num = int(parts[6])

	if modifier_check == "==":
		if registers[reg_to] == modifier_check_num:
			if instruction == "dec":
				registers[reg_from] -= instruction_check_num
			else:
				registers[reg_from] += instruction_check_num
	elif modifier_check == "<=":
		if registers[reg_to] <= modifier_check_num:
		...
		if not registers[reg_to] == modifier_check_num:
			if instruction == "dec":
				registers[reg_from] -= instruction_check_num
			else:
				registers[reg_from] += instruction_check_num
		...
		if registers[reg_to] > modifier_check_num:
			if instruction == "dec":
				...
				registers[reg_from] += instruction_check_num
	if registers[reg_from] > max:
		max = registers[reg_from]
